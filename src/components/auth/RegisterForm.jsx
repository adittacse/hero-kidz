...
export const RegisterForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!form.email.endsWith("gmail.com")) {
            await Swal.fire("We only Accept Gmail", "Gmail একাউণ্ট ব্যবহার করুন", "error");
            setLoading(false);
            return;
        }

        if (form.password.length < 6) {
            await Swal.fire("Week Password", "Stong পাসওয়ার্ড দিন", "error");
            setLoading(false);
            return;
        }
        ...
        if (result.acknowledged) {
            ...
            await Swal.fire("error", "এই gmail এ  একটি একাউন্ট আছে । লগিন করুন ", "error");
            setLoading(false);
        }
    };
